...
/*   Updated: 2023/11/17 12:21:41 by enaam            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	_g->texture->xpm[0] = mlx_xpm_file_to_image(_g->mlx_s->mlx_ptr, _g->maps->north, &_g->texture->x_width, &_g->texture->x_hight);
	_g->texture->xpm[1] = mlx_xpm_file_to_image(_g->mlx_s->mlx_ptr, _g->maps->south, &_g->texture->x_width, &_g->texture->x_hight);
	_g->texture->xpm[2] = mlx_xpm_file_to_image(_g->mlx_s->mlx_ptr, _g->maps->east, &_g->texture->x_width, &_g->texture->x_hight);
	_g->texture->xpm[3] = mlx_xpm_file_to_image(_g->mlx_s->mlx_ptr, _g->maps->west, &_g->texture->x_width, &_g->texture->x_hight);
	_g->texture->addr_x[0] = mlx_get_data_addr(_g->texture->xpm[0], &(_g->texture->b_p_x), &(_g->texture->l_x), &(_g->mlx_s->dian));
	_g->texture->addr_x[1] = mlx_get_data_addr(_g->texture->xpm[1], &(_g->texture->b_p_x), &(_g->texture->l_x), &(_g->mlx_s->dian));
...
unsigned int	get_pixel(t_global *_g, int x, int y, int i)
{
	...
	{
		dst = _g->texture->addr_x[i] + (y * _g->texture->l_x + x * (_g->texture->b_p_x / 8));
...
void	texture_offset(t_global *_g, int i)
{
	...
	while (j < _g->texture->wallbotmpixl)
	{
		...
		_g->texture->textureoffsety = (disftop *  (float)GRID_SIZE ) / _g->texture->wallstripht;
...
{
	_g->texture->per_dist = _g->rays[i]->distance * cos(_g->rays[i]->angle_veiw - _g->player->rotation_angle);
	_g->texture->distpropln = (WIDTH / 2) / tan(FOV_ANGLE / 2);
	_g->texture->projwallht = (GRID_SIZE / _g->texture->per_dist) * _g->texture->distpropln;
