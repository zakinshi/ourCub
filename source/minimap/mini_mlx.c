...
/*   Updated: 2023/11/20 18:55:25 by zakbouha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mn_mlx_s->mn_img = mlx_new_image(mn_mlx_s->mlx_ptr, MINI_WIDTH, MINI_HEIGHT);
	if (!mn_mlx_s->mn_img)
		exit_msg("Allocation Failed for mn_mlx_s->mn_img");
	mn_mlx_s->mn_addr = mlx_get_data_addr(mn_mlx_s->mn_img, &(mn_mlx_s->b_p_l), &(mn_mlx_s->l_l), &(mn_mlx_s->dian));
	if (!mn_mlx_s->mn_addr)
